  ...
  async deleteStatusDoc(req, res) {
    ...
    try {
      ...
      await BooksRegisInfo.update(
        { StatusDocId: 1, Status: 0 },
        { where: { StatusDocId: statusId } },
      );
      ...
      return res.json({
        success:
          'Xóa trạng thái thành công! Các phân phối sách thuộc trạng thái này sẽ được chuyển thành "Không xác định"',
      });
    } catch (error) {
      return res.json({
        error: "Đã xảy ra lỗi khi xóa trạng thái. Vui lòng thử lại sau.",
      });
    }
  }
}
