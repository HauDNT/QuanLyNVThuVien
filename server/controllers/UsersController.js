...
class UsersController {
    ...
    async createInfoUser(req, res) {
        try {
            // Lấy thông tin cá nhân đã được gửi từ client:
            const {
                username,
                fullname,
                email,
            ...
            // Tạo thông tin cá nhân và quyền hạn tài khoản:
    ...
    async updateAccount(req, res) {
        ...
        try {
            ...
            const fieldsChange = Object.keys(data); // Mảng chứa các tên thuộc tính (key)
            const valuesChange = Object.values(data); // Mảng chứa các giá trị của các thuộc tính (value)

            for (let i = 0; i < fieldsChange.length; i++) {
                let attributesUpdating = {};
                if (
                    fieldsChange[i] !== 'NewPassword' &&
                    fieldsChange[i] !== 'RoleId'
                ) {
                    attributesUpdating[fieldsChange[i]] = valuesChange[i]; // Gán giá trị tương ứng từ valuesChange vào đối tượng attributesUpdating
                    UsersInfo.update(attributesUpdating, {
                        // Đưa object attributesUpdating chứa dữ liệu vào insert vô DB
                        where: { UserId: userId },
                    });
                }
            }
    ...
    async getAccountSoftDeleted(req, res) {
        try {
            const accountDeleted = await Users.findAll({
                attributes: [
                    'id',
                    'Username',
                    'Password',
                ...
                where: {
                    deletedAt: {
                        [Op.ne]: null, // Điều kiện deletedAt không bằng null, tức là chỉ lấy các bản ghi đã bị xóa mềm.
                    },
                },
                paranoid: false, // Khi paranoid được đặt thành false, các bản ghi bị xóa mềm vẫn có thể được truy vấn.
            });

            const data = accountDeleted.map((item) => ({
                id: item.id,
