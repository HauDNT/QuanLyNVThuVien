...
class PositionsController {
    ...
    async createPosition(req, res) {
        try {
            ...
            await Positions.create({PositionName: positionName, Description: description});
    ...
    async deletePosition(req, res) {
        ...
        try {
            ...
            return res.json({success: 'Xóa chức vụ thành công! Các tài khoản thuộc chức vụ này sẽ được chuyển thành "Không xác định"'});
        } catch (error) {
            return res.json({error: 'Đã xảy ra lỗi khi xóa chức vụ. Vui lòng thử lại sau.'})
        }
    };
}
