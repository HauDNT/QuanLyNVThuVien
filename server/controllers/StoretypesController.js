  ...
  async deleteStoreType(req, res) {
    ...
    try {
      ...
      await BooksRegisInfo.update(
        { StoreTypeId: 1, Status: 0 },
        { where: { StoreTypeId: typeId } },
      );
      ...
      return res.json({
        success:
          'Xóa thể loại thành công! Các phân phối sách thuộc thể loại này sẽ được chuyển thành "Không xác định"',
      });
    } catch (error) {
      return res.json({
        error: "Đã xảy ra lỗi khi xóa thể loại. Vui lòng thử lại sau.",
      });
    }
  }
}
