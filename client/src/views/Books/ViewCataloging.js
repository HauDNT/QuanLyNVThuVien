      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="view-cataloging--container">
            <h4 className="col-12 view-cataloging--header">
              Danh sách biên mục
            </h4>
            {listCataloging &&
              listCataloging.map((catalog) => {
                ...
                const countNotAccept =
                  catalog.BooksRegisInfos.filter((info) => !info.Status)
                    .length - 1;
                    ...
                    <div className="row div-list-item">
                      <div className="col-lg-2 col-md-2">
                        <div className="row area-1">
                          <div className="text-center order-number">
                            {catalog.id}
                          </div>
                          <div className="text-center classification">
                            {catalog.DDC}
                          </div>
                          <div className="text-center encrypt-name">
                            {catalog.EncryptName}
                          </div>
                        </div>
                      </div>
                      <div className="col-lg-10 col-lg-10">
                        <div className="row">
                          ...
                          <div className="col-md-8 col-lg-8 area-2">
                            <Link
                              to={`/book/cataloging/edit/${catalog.id}`}
                              className="book-name"
                            >
                              {catalog.MainTitle}
                              {" - " + catalog.Author}
                            ...
                            <p className="publisher">
                              Chịu trách nhiệm: {catalog.Editors}
                            </p>
                            <p className="publisher">
                              Nhà xuất bản: {catalog.Publisher}
                            </p>
                            <p className="publish-year">
                              Năm xuất bản: {catalog.PubYear}
                            </p>
                          </div>
                            ...
                            <div className="row">
                              ...
                              <div className="status">
                                ...
                                {catalog.BooksRegisInfos.length === 1 ? (
                                  <span className="span--status status-0">
                                    Chưa duyệt ({countNotAccept})
                                  </span>
                                ) : isNotAccept ? (
                                  <span className="span--status status-0">
                                    Chưa duyệt ({countNotAccept})
                                  ...
                                  <span className="span--status status-1">
                                    Đã duyệt
                                  </span>
                                )}
                              </div>
