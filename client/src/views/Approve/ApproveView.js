...
function ApproveView() {
  ...
  const handleCheck = (event) => {
    ...
    if (checked) {
      ...
      setApproveSelected(
        approveSelected.filter((approveId) => approveId !== +value),
      );
    }
  };
  ...
  const handleCheckAll = (event) => {
    ...
    allChildCheckboxes.forEach((eachCheckbox) => {
      ...
      if (checked) {
        ...
        setApproveSelected(
          approveSelected.filter(
            (approveId) => approveId !== +eachCheckbox.value,
          ),
        );
      }
    });
  ...
  const handleDeleteApprove = () => {
    const deletePromises = approveSelected.map((approveId) => {
      return axios.delete(`http://${config.URL}/approve/delete/${approveId}`, {
        headers: {
    ...
    Promise.all(deletePromises).then((res) => {
      ...
      res.forEach((res, index) => {
        ...
        if (!res.data.error) {
          setListApproveInfo((oldList) =>
            oldList.filter((approve) => approve.id !== eachApproveSelected),
          );
          status = true;
        }
      });
      ...
      } else toast.error("Xóa phân phối không thành công!");
    });
  };
                ...
                {records && records.length > 0 ? (
                  records.map((approve) => (
                    <tr key={approve.id} className="text-center">
                      ...
                      <td>
                        <Link
                          className="link-update-approve"
                          to={`/approve/update/${approve.id}`}
                        >
                          {approve.RegisCode}
                        </Link>
                      </td>
