...
function ApproveCreate() {
    ...
    axios
      ...
      .then((res) => {
        if (res.data.success) {
          // Send data to Server:
          axios
            .post(`http://${config.URL}/approve/create/:${bookId}`, data, {
              headers: {
  ...
  return (
    <form
      method="POST"
      className="container--approve-create"
              ...
              {listBills ? (
                listBills.map((bill) => (
                  <option value={bill.id}>
                    {" "}
                    {formatAndDisplayDatetime(bill.createdAt)} - {bill.NameBill}
                  </option>
                ))
              ) : (
                <option value="0" selected>
        ...
        <div className="row mb-3">
          <div className="col">
            ...
            <textarea
              className="form-control"
              rows="5"
              value={initValues.AllRegisCode.map((regisCode) => regisCode).join(
                " - ",
              )}
              readOnly
            ></textarea>
