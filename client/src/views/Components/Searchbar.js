...
const Searchbar = ({
  ...
  const handleSearch = () => {
      ...
      switch (searchType) {
        case "books":
          ...
          if (selectedCategory === "*") {
            ...
            axios
              .get(`http://${config.URL}/books/getAccessCatalog`, {
                headers: { authenToken: localStorage.getItem("authenToken") },
              })
              .then((res) => {
                if (!res.data.error) {
            ...
            axios
              .get(`http://${config.URL}/books/getNotAccessCatalog`, {
                headers: { authenToken: localStorage.getItem("authenToken") },
              })
              .then((res) => {
                if (!res.data.error) {
                  const filterData = res.data.filter(
                    (result) => result.BooksRegisInfos.length > 1,
                  );
            ...
            axios
              .post(`http://${config.URL}/books/search`, data, {
                headers: { authenToken: localStorage.getItem("authenToken") },
              })
              .then((res) => {
                if (!res.data.error) {
        ...
        case "bills":
          ...
          if (selectedCategory === "*") {
            ...
            axios
              .post(`http://${config.URL}/bills/searchBills`, data, {
                headers: { authenToken: localStorage.getItem("authenToken") },
              })
              .then((res) => {
                if (!res.data.error) {
        ...
        case "booksOfBill":
          ...
          if (selectedCategory === "*") {
            ...
            axios
              .post(`http://${config.URL}/bills/searchBooksOfBill`, data, {
                headers: { authenToken: localStorage.getItem("authenToken") },
              })
              .then((res) => {
                if (!res.data.error) {
        ...
        case "approve":
          ...
          if (selectedCategory === "*") {
            ...
            axios
              .post(`http://${config.URL}/approve/searchInApprove`, data, {
                headers: { authenToken: localStorage.getItem("authenToken") },
              })
              .then((res) => {
                if (!res.data.error) {
        ...
        case "users":
          ...
          if (selectedCategory === "*") {
            ...
            axios
              .post(`http://${config.URL}/users/search`, data, {
                headers: { authenToken: localStorage.getItem("authenToken") },
              })
              .then((res) => {
                if (!res.data.error) {
        ...
        case "encodeTitles":
          ...
          if (selectedCategory === "*") {
            axios
              ...
              .catch((error) =>
                toast.error("Không thể tải dữ liệu lên!", error),
              );
          } else {
            axios
              .post(`http://${config.URL}/encodeTitles/search`, data, {
                headers: { authenToken: localStorage.getItem("authenToken") },
              })
              .then((res) => {
                if (!res.data.error) {
    ...
    } else toast.warning("Bạn phải chọn hạng mục và nội dung để tìm kiếm!");
