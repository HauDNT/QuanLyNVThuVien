...
function BillTypes() {
    ...
    const renderBillTypes = () => {
        axios
        .get(`http://${config.URL}/bills/getAllBillTypes`)
        .then((res) => {
    ...
    const addType = (values) => {
        axios
            .post(
                `http://${config.URL}/bills/addBillType`, 
                values, 
                { headers: {
    ...
    const deleteType = (typeId) => {
        axios
            .delete(
                `http://${config.URL}/bills/deleteType/${typeId}`,
                { headers: {
                    authenToken: localStorage.getItem('authenToken')
                }}
            )
            ...
            .catch(error => {
                toast.error("Đã xảy ra lỗi trong quá trình xóa loại hóa đơn!", error);
            })
    };
                ...
                >
                    {({ values, setFieldValue }) => (
                        <Form className="col-5 form-add-type">
                            <div className="mb-3">
                                <h5 className="form-label mb-3 text-center">Thêm loại hóa đơn</h5>
                                ...
                                <Field 
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setInitValues({...values, typeId: +e.target.value});
                                    }}
                                >
                                    <option value="1">Mua</option>
                                    <option value="2">Nhận / biếu tặng</option>
                                </Field>
                                ...
                                <Field
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setInitValues({...values, typeName: e.target.value});
                                    }}
                    ...
                    <table className="styled-table table--bill-types">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" className="text-center"> Tên loại </th>
