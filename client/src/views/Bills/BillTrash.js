...
function BillTrash() {
  ...
  const formatAndDisplayDatetime = (dateString) => {
    ...
    dateString = `${String(date.getHours()).padStart(2, "0")}:${String(date.getMinutes()).padStart(2, "0")}:${String(date.getSeconds()).padStart(2, "0")}
  ...
  const handleCheckAll = (event) => {
    ...
    allChildCheckboxes.forEach((eachCheckbox) => {
      ...
      if (checked) {
        ...
        setBillSelected(
          billSelected.filter((billId) => billId !== +eachCheckbox.value),
        );
      }
    });
